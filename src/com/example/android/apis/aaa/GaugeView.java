...
public class GaugeView extends View {
    ...
    public static final float[] RANGE_VALUES = {16.0f, 25.0f, 40.0f, 260.0f};
	...
	private RectF mInnerRect1;
	private RectF mInnerRect2;
	...
	private void readAttrs(final Context context, final AttributeSet attrs, final int defStyle) {
		...
		mNeedleWidth = NEEDLE_WIDTH;
		mNeedleHeight = NEEDLE_HEIGHT;
	...
	public void initDrawingRects() {
		...
		float borderWidth = 0.18f;
		mInnerRect1 = new RectF(mOuterShadowRect.left + borderWidth, mOuterShadowRect.top + borderWidth,
                mOuterShadowRect.right - borderWidth, mOuterShadowRect.bottom - borderWidth);
		borderWidth = 0.15f;
		mInnerRect2 = new RectF(mOuterShadowRect.left + borderWidth, mOuterShadowRect.top + borderWidth,
                mOuterShadowRect.right - borderWidth, mOuterShadowRect.bottom - borderWidth);
		Log.e("View", "----------"+mInnerRect1.toShortString());
...
//			canvas.drawCircle(0.5f, 0.5f, 0.04f, mNeedleScrewBorderPaint);
	        ...
	        final Paint paint = new Paint(Paint.ANTI_ALIAS_FLAG);
	        paint.setStyle(Paint.Style.STROKE);
	        paint.setColor(Color.argb(255, 6, 72, 130));
	        paint.setStrokeWidth(0.08f);
	        final Paint paint1 = new Paint(Paint.ANTI_ALIAS_FLAG);
	        paint1.setStyle(Paint.Style.STROKE);
	        paint1.setColor(Color.argb(100, 83, 175, 255));
	        paint1.setStrokeWidth(0.09f);
	        float start = getId()==R.id.speed_gauge_view?50:100;
	        float sweepAngle = getId()==R.id.speed_gauge_view?mTargetValue:-(mTargetValue/50);
	        canvas.drawArc(mInnerRect2, //弧线所使用的矩形区域大小   
                    start,  //开始角度   
                    sweepAngle, //扫过的角度   
                    false, //是否使用中心   
                    paint1);  
	        canvas.drawArc(mInnerRect1, //弧线所使用的矩形区域大小   
	                start,  //开始角度   
	                sweepAngle, //扫过的角度   
	                false, //是否使用中心   
	                paint);  
	        
	        Log.e("View", "-------mScaleStartAngle ="+mScaleStartAngle+" angle="+angle);
